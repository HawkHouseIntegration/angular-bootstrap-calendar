...
describe('mwlCalendarWeek directive', function() {
  ...
  it('should get the new week view when calendar refreshes', function() {
    ...
    expect(calendarHelper.getWeekView).to.have.been.calledWith(scope.events, scope.viewDate, false);
    expect(MwlCalendarCtrl.view).to.eql({events: [{title: 'event1'}, {title: 'oneDayEvent'}]});

    MwlCalendarCtrl.showTimes = true;
    scope.$broadcast('calendar.refreshView');
    expect(calendarHelper.getWeekView).to.have.been.calledWith(scope.events, scope.viewDate, true);
